...
Before deploying, edit `frontend/public/_redirects` — replace `YOUR-DOMAIN.ca` with your actual domain:

```
/api/* https://api.YOUR-DOMAIN.ca/:splat 200
```
...
Edit `cloudflare/tunnel-config.yml` — replace `<TUNNEL_ID>` and `YOUR-DOMAIN.ca`:
...
```bash
cloudflared tunnel route dns mcfd-files api.YOUR-DOMAIN.ca
```
...
```bash
# Health check
curl https://api.YOUR-DOMAIN.ca/api/health

# Public endpoint — no key
curl https://api.YOUR-DOMAIN.ca/api/costs

# Protected endpoint — no key (should return 401)
curl https://api.YOUR-DOMAIN.ca/api/contradictions

# Protected endpoint — with key (should return 200)
curl -H "X-API-Key: <your-key>" https://api.YOUR-DOMAIN.ca/api/contradictions
```
...
- [ ] Frontend loads at `https://YOUR-DOMAIN.ca`
